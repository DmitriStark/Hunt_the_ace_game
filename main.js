...
const numCards = cardObjectDefinnitions.length
...
function gameOver() {
    ...
    const gameOverMessage = `Game Over!  final score -<span class ='badge'>${score}</span>
                             Click play button to play again!`;
...
function outputChoiseFeedBack(hit) {
    if (hit) {
...
function evaluateCardChoise(card) {
    if (card.id == aceId) {
        updateScore()
        outputChoiseFeedBack(true)

    }
    else {
        outputChoiseFeedBack(false)
...
function shuffleCards() {
    ...
    function shuffle() {
        ...
        if (shuffleCount == 500) {
            ...
            dealCards()
            updateStatusElement(currentGameStatusElem, "block", primaryColor, "Please click on card that you thing is the Ace of Spades...")
        }
        else {
            shuffleCount++
        }
...
function createCards() {
    cardObjectDefinnitions.forEach((cardItem) => {
        createCard(cardItem)
    })
...
function createCard(cardItem) {
    ...
    //create front and back image elemnts for card
    const cardFrontImg = createElement('img')
    const cardbackdImg = createElement('img')
    ...
    //add class to front element
    addClassToElement(cardFrontelem, 'card-front')

    //add class to back element
    addClassToElement(cardBackElem, "card-back")

    //add src attribute and approriate value to img element - back of card
    addSrcToImageElem(cardbackdImg, cardBackImgPath)

    //add src attribute and approriate value to img element -front of card
    addSrcToImageElem(cardFrontImg, cardItem.imagePath)

    //assign class to back image element of back card
    addClassToElement(cardbackdImg, "card-img")

    //assign class to front image element of front card
    addClassToElement(cardFrontImg, "card-img")

    //add back immage element as child element to front card element
    addChildElement(cardFrontelem, cardFrontImg)

    //add back immage element as child element to back card element
    addChildElement(cardBackElem, cardbackdImg)
    ...
    //add card element as child element to appriate grid cell
